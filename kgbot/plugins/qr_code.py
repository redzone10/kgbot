...
@kgbot.on(admin_cmd(pattern="getqr"))
@kgbot.on(sudo_cmd(pattern="getqr", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    start = datetime.now()
...
@kgbot.on(admin_cmd(pattern="makeqr ?(.*)"))
@kgbot.on(sudo_cmd(pattern="makeqr ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    start = datetime.now()
