...
from kgbot import ALIVE_NAME, CMD_HELP, thunderversion
...
@kgbot.on(admin_cmd(outgoing=True, pattern="alive"))
@kgbot.on(sudo_cmd(outgoing=True, pattern="alive", allow_sudo=True))
async def ifiamalive(alive):
    ...
    if ALV_PIC:
        ...
        thundrgang += "    [GitHub Repo](https://github.com/kgproject1010/kgbot)"
    ...
    with BytesIO() as sticker:
        ...
        await borg.send_message(
            ...
            f"{alivemoji} **kgbot Version**: `{thunderversion}`\n"
            f"{alivemoji} **More Info**: @kgbot\n"
            f"{alivemoji} **Sudo** : `{sudo}`\n"
            f"{alivemoji} **kgbot Uptime**: `{uptime}`\n"
            f"{alivemoji} **Database Status**: `Everything Stable As Lightning⚡️⚡️`\n"
            f"{alivemoji} **My pro owner** : [{DEFAULTUSER}](tg://user?id={myid})\n\n"
            "    [GitHub Repo](https://github.com/kgproject1010/kgbot)",
            link_preview=False,
        )
