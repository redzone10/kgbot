...
import kgbot.plugins.sql_helper.pmpermit_sql as pmpermit_sql
from kgbot import ALIVE_NAME, CMD_HELP, CUSTOM_PMPERMIT, bot
...
MESAG = (
    str(CUSTOM_PMPERMIT)
    if CUSTOM_PMPERMIT
    else "`kgbot PM security! Please wait for me to approve you. 😊"
)
...
@kgbot.on(admin_cmd(pattern="a ?(.*)"))
@kgbot.on(admin_cmd(pattern="approve ?(.*)"))
...
@kgbot.on(admin_cmd(pattern="block ?(.*)"))
async def approve_p_m(event):
    if event.fwd_from:
        return
    replied_user = await event.client(GetFullUserRequest(event.chat_id))
...
@kgbot.on(admin_cmd(pattern="da ?(.*)"))
@kgbot.on(admin_cmd(pattern="disapprove ?(.*)"))
async def approve_p_m(event):
    if event.fwd_from:
        return
    replied_user = await event.client(GetFullUserRequest(event.chat_id))
...
@kgbot.on(admin_cmd(pattern="listapproved"))
async def approve_p_m(event):
    if event.fwd_from:
        return
    approved_users = pmpermit_sql.get_all_approved()
    APPROVED_PMs = "[kgbot] Currently Approved PMs\n"
    ...
    if len(APPROVED_PMs) > 4095:
        with io.BytesIO(str.encode(APPROVED_PMs)) as out_file:
            ...
            await event.client.send_file(
                ...
                force_document=True,
                allow_cache=False,
                caption="[kgbot]Current Approved PMs",
                reply_to=event,
            )
            await event.delete()
    else:
...
if NEEDIT == "on":
    ...
    @kgbot.on(events.NewMessage(incoming=True))
    async def on_new_private_message(event):
