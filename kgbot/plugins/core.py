...
from kgbot.utils import admin_cmd, load_module, remove_plugin
...
@kgbot.on(admin_cmd(pattern=r"send (?P<shortname>\w+)", outgoing=True))
@kgbot.on(sudo_cmd(pattern=r"send (?P<shortname>\w+)", allow_sudo=True))
async def send(event):
    ...
    the_plugin_file = "./kgbot/plugins/{}.py".format(input_str)
...
@kgbot.on(admin_cmd(pattern="install"))
        ...
        try:
            downloaded_file_name = (
                await event.client.download_media(  # pylint:disable=E0602
                    ...
                    "kgbot/plugins/",  # pylint:disable=E0602
                )
            )
            if "(" not in downloaded_file_name:
                ...
                await event.edit(
                    "kgbot Succesfully Installed The Plugin `{}`".format(
...
@kgbot.on(admin_cmd(pattern=r"unload (?P<shortname>\w+)$"))
...
@kgbot.on(admin_cmd(pattern=r"load (?P<shortname>\w+)$"))
async def load(event):
    ...
    try:
        ...
        await event.edit(f"kgbot has successfully loaded {shortname}")
    except Exception as e:
        await event.edit(
            f"kgbot could not load {shortname} because of the following error.\n{str(e)}"
        )


@kgbot.on(admin_cmd(pattern=r"installall$"))
async def install(event):
    if event.fwd_from:
        return
    ...
    for ixo in total_doxx:
        ...
        downloaded_file_name = await event.client.download_media(
            await event.client.get_messages(event.chat_id, ids=mxo), "kgbot/plugins/"
        )
        if "(" not in downloaded_file_name:
            path1 = Path(downloaded_file_name)
            shortname = path1.stem
