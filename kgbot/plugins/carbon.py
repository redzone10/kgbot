...
@kgbot.on(admin_cmd(pattern="carbon"))
@kgbot.on(sudo_cmd(pattern="carbon", allow_sudo=True))
async def carbon_api(e):
    ...
    if not e.text[0].isalpha() and e.text[0] not in ("/", "#", "@", "!"):
        ...
        driver = webdriver.Chrome(
            executable_path=THECHROME_DRIVER, options=chrome_options
        )
        ...
        await e.client.send_file(
            e.chat_id,
            file,
            caption="**Done**, \n ⚡️ Carbonised by ⚡️ [kgbot](https://t.me/kgbot) ",
            force_document=True,
