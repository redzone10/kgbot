...
@kgbot.on(admin_cmd(pattern="time ?(.*)"))  # pylint:disable=E0602
@kgbot.on(sudo_cmd(pattern="time ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
...
@kgbot.on(admin_cmd(pattern="gtime (.*)"))  # pylint:disable=E0602
@kgbot.on(sudo_cmd(pattern="gtime (.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    input_str = event.pattern_match.group(1)
