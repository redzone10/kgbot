...
async def _(event):
    ...
    if event.reply_to_msg_id:
        ...
        if previous_message.media:
            downloaded_file_name = await kgbot.download_media(
    ...
    if not len(fedList):
        for a in range(3):
            async with kgbot.conversation("@MissRose_bot") as bot_conv:
    ...
    try:
        await kgbot.send_message(chat, f"/start")
    except BaseException:
    ...
    for fed in fedList:
        if Config.ALL_FEDS and fed in excludeFed:
            await kgbot.send_message(chat, f"{fed} Excluded.")
            exCount += 1
            continue
        await kgbot.send_message(chat, f"/joinfed {fed}")
        await asyncio.sleep(3)
        await kgbot.send_message(chat, f"/fban {FBAN} {REASON}")
        await asyncio.sleep(3)
    await event.edit(
