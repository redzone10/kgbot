...
from kgbot import CMD_HELP, LOGS
from kgbot.utils import admin_cmd, humanbytes, time_formatter
...
@kgbot.on(admin_cmd(pattern=f"gdl", outgoing=True))
@kgbot.on(sudo_cmd(pattern=f"gdl", allow_sudo=True))
...
@kgbot.on(admin_cmd(outgoing=True, pattern=r"mega(?: |$)(.*)"))
