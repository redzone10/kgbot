...
@kgbot.on(admin_cmd(pattern="botgban ?(.*)"))
@kgbot.on(sudo_cmd(pattern="botgban ?(.*)", allow_sudo=True))
async def _(event):
...
@kgbot.on(admin_cmd(pattern="botungban ?(.*)"))
@kgbot.on(sudo_cmd(pattern="botungban ?(.*)", allow_sudo=True))
async def _(event):
    if Config.G_BAN_LOGGER_GROUP is None:
        await eor(
            event,
