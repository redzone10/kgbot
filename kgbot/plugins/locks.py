...
from kgbot.plugins.sql_helper.locks_sql import get_locks, is_locked, update_lock
from kgbot.utils import admin_cmd


@kgbot.on(admin_cmd(pattern=r"lock( (?P<target>\S+)|$)"))
@kgbot.on(sudo_cmd(pattern=r"lock( (?P<target>\S+)|$)", allow_sudo=True))
async def _(event):
...
@kgbot.on(admin_cmd(pattern="unlock ?(.*)"))
@kgbot.on(sudo_cmd(pattern="unlock ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
...
@kgbot.on(admin_cmd(pattern="currenabledlocks"))
@kgbot.on(sudo_cmd(pattern="currenabledlocks", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    res = ""
...
@kgbot.on(events.MessageEdited())  # pylint:disable=E0602
@kgbot.on(events.NewMessage())  # pylint:disable=E0602
...
@kgbot.on(events.ChatAction())  # pylint:disable=E0602
async def _(event):
    # TODO: exempt admins from locks
    # check for "lock" "bots"
