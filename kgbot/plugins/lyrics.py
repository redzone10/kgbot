...
@kgbot.on(admin_cmd(pattern="lyrics ?(.*)"))
@kgbot.on(sudo_cmd(pattern="lyrics ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    ...
    if queryinput == "":
        await ok.edit("It Should Be In This Format`.lyrics`songname")
        return
...
CMD_HELP.update({"lyrics": "➟ `.lyrics` songname\nUse - Get Lyrics Of Any Song"})
