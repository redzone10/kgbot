...
async def handler(thundrgang):
    if thundrgang.user_joined or thundrgang.user_added:
        try:
            from kgbot.plugins.sql_helper.gmute_sql import is_gmuted
...
@kgbot.on(admin_cmd(pattern="gban(?: |$)(.*)"))
async def gspider(rk):
    ...
    if user:
        ...
        try:
            from kgbot.plugins.sql_helper.gmute_sql import gmute
        except BaseException:
            pass
        try:
...
@kgbot.on(admin_cmd(pattern="ungban(?: |$)(.*)"))
async def gspider(rk):
    lazy = rk
    sender = await lazy.get_sender()
    me = await lazy.client.get_me()
    ...
    if user:
        ...
        try:
            from kgbot.plugins.sql_helper.gmute_sql import ungmute
        except BaseException:
            pass
        try:
