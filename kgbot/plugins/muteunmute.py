import asyncio
from kgbot.plugins.sql_helper.mute_sql import is_muted, mute, unmute
from kgbot.utils import admin_cmd, errors_handler, sudo_cmd
from kgbot import CMD_HELP


@kgbot.on(admin_cmd(pattern="mute ?(\d+)?"))
@kgbot.on(sudo_cmd(outgoing=True, pattern="mute ?(\d+)?", allow_sudo=True))
...
@kgbot.on(admin_cmd(pattern="unmute ?(\d+)?"))
@kgbot.on(sudo_cmd(outgoing=True, pattern="unmute ?(\d+)?", allow_sudo=True))
