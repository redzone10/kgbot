import re
from kgbot.plugins.lightningbot import *
from telethon import events, Button
import heroku3
import asyncio
import os
import requests
from kgbot.plugins.lightningbot.sql.blacklist_sql import all_bl_users
from kgbot.plugins import THEFIRST_NAME
from kgbot.plugins.lightningbot.sql.userbase_sql import add_to_userbase, present_in_userbase, full_userbase
from datetime import datetime
from telethon import events
from kgbot.kgbotConfig import Var, Config
from telegraph import Telegraph, upload_file
from kgbot import CUSTOM_PMPERMIT
    ...
    if LOAD_MYBOT == "False":
        ...
        if BOT_PIC:
            await tgbot.send_file(event.chat_id,
                                  ...
                                  buttons=[
                                      [Button.url(
                                          "kgbot", url="https://github.com/kgproject1010/kgbot")],
                                      [Button.inline(
                                          "Whats this?", data="kgbot")]
                                  ]
                                  )
        else:
            await tgbot.send_message(event.chat_id,
...
async def logs(event):
    ...
    await tgbot.send_file(
        ...
        "logs.txt",
        reply_to=event.id,
        caption="**Heroku** kgbot Logs",
        buttons=[
            [Button.url("View Online", f"{url}")],
            [Button.url("Crashed?", "t.me/kgbot")]
        ])
...
@tgbot.on(events.callbackquery.CallbackQuery(data=re.compile(b"kgbot"))
          )  # pylint: disable=oof
async def settings(event):
    await event.delete()
    await tgbot.send_message(event.chat_id,
...
async def settings(event):
    await event.edit("Browse through the available options:",
                     buttons=[
                         [(Button.url("Repository", url="https://github.com/kgproject1010/kgbot")),
                          (Button.url("Deploy Your Own", url="https://t.me/kgbot"))],
                         [Button.url("Support",
                                     url="https://t.me/kgbot")]
                     ])
